    ...
    <div className="flex flex-col h-full">
      ...
      <div className="overflow-y-auto flex-1">
        ...
        {!hasMore && notifications.length > 0 && (
          <p className="text-center text-xs text-gray-400 py-4">Fim das notificações</p>
        )}
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-1">
          <p className={`text-sm leading-snug ${notification.is_read ? 'text-gray-600 font-normal' : 'text-gray-900 font-medium'}`}>
            {notification.title}
          </p>
          {!notification.is_read && (
            <span aria-hidden="true" className="flex-shrink-0 mt-1 h-2 w-2 rounded-full bg-blue-500" />
          )}
        </div>
